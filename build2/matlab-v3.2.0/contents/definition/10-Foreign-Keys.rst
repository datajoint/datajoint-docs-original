...
Even if you already know about foreign keys in SQL, please still read this section carefully.
...
The graph defines the direction of the workflow. The tables at the top of the flow need to be populated first, followed by table on step below and so forth until the last table is populated at the bottom of the pipeline.
...
.. note::
   ...
   In either case, the foreign key always points to upstream upstream tables.
...
This displayed heading reflects the actual columns in the table.
The foreign keys have been replaced by the primary key attributes of the referenced tables, including their data type and comment.
...
A foreign key constraint means that a tuple in ``B`` cannot exist without a matching tuple in ``A``.
...
--------------------
In most cases, the foreign key includes the primary key attributes of the referenced table in the table definition without renaming them.
In such a case, an entity in the dependent table depends on exactly one entity in the referenced table.
DataJoint provides the following syntax rename the primary key attributes when they are included in the new table.
...
Note that the second foreign key could just as well have been ``(postsynaptic) -> Cell`` with the same effect but it does not make the table definition any clearer.
...
If we wished to allow to represent synapses between cells from different slices, then we would have to rename ``slice_id`` as well:
...
This primary key still imposes the constraint that synapses can only form between cells within the same animal but may connect cells across different slices.
