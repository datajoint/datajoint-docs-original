...
if os.path.exists('build-all'):
    shutil.rmtree('build-all')

os.makedirs('build-all')
subprocess.Popen(
    ["git", "clone", "git@github.com:mahos/testDocMain.git", "datajoint-docs"], cwd="build-all").wait()

subprocess.Popen(
    ["git", "clone", "git@github.com:mahos/testDocMatlab.git", "datajoint-matlab"], cwd="build-all").wait()

subprocess.Popen(
    ["git", "clone", "git@github.com:mahos/testDocPython.git", "datajoint-python"], cwd="build-all").wait()
    

def create_build_folders(lang): 
    raw_tags = subprocess.Popen(["git", "tag"], cwd="build-all/datajoint-" + lang, stdout=subprocess.PIPE).communicate()[0].decode("utf-8").split()
    git_tags = {}
    ...
    for tag in tags:
        subprocess.Popen(["git", "checkout", tag],
                         cwd="build-all/datajoint-" + lang, stdout=subprocess.PIPE).wait()
        dsrc_lang2 = "build-all/datajoint-" + lang + "/docs"
        dst_build_folder = "build-all/" + lang + "-" + tag 
        ...
        subprocess.Popen(["git", "checkout", version_info['comm_version']],
                         cwd="build-all/datajoint-docs", stdout=subprocess.PIPE).wait()
        dsrc_comm2 = "build-all/datajoint-docs/contents"
...
def make_full_site():
    ...
    to_make = [folder for folder in glob.glob('build-all/**') if not os.path.basename(folder).startswith('datajoint')]
    # print(to_make)
