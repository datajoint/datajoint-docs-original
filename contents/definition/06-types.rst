...
-  ``enum``: one of several explicitly enumerated values specified as strings.
   ...
   For example, for anesthesia, the datatype could be ``enum("urethane", "isoflurane", "fentanyl")``.
...
-  ``longblob``: arbitrary MATLAB value (e.g. matrix, image, structure), up to 4 `GiB <http://en.wikipedia.org/wiki/Gibibyte>`__ in size.
   In Python, arbitrary numeric numpy array.
   Numeric arrays are compatible between MATLAB and Python.
...
-  ``mediumblob``: arbitrary MATLAB value, up to 16 `MiB <http://en.wikipedia.org/wiki/Mibibyte>`__
-  ``blob``: arbitrary MATLAB value, up to 64 `KiB <http://en.wikipedia.org/wiki/Kibibyte>`__
