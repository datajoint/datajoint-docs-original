...
.. include:: ../admin/5-blob-config_lang5.rst
...
.. note::
  ...
  Filepath Datatype is available as a preview feature in DataJoint Python v0.12. This means that the feature is required to be explicitly enabled. To do so, make sure to set the `environment variable <https://en.wikipedia.org/wiki/Environment_variable>`_ ``FILEPATH_FEATURE_SWITCH=TRUE`` prior to use.
...
It is possible to skip the checksum but it is not recommended as you no longer
have the integrity guarantee, you should only do so if you are experiencing significant
delays in fetching large files. To disable checksums you can set a threshold in bytes
for when to stop using checksums like in the example below:

.. code-block:: python

    dj.config["filepath_checksum_size_limit"] = 1024*1024*1024 # This is 1GB so all files over that will not be checked
